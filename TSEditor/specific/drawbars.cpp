...
static void S_DrawHealthBar2(long pos)
{
	...
	h = GetRenderScale(6);
	x = phd_centerx - GetRenderScale(75);
...
long S_DrawLoadBar()
{
	...
	if (tomb5.tr4_loadbar)
	{
		...
		w = phd_winwidth - (x << 1);
		h = GetRenderScale(7);
		y = phd_winheight - h - GetRenderScale(20);
...
void DoSlider(long x, long y, long width, long height, long pos, long c1, long c2, long c3)
{
	...
	sy = (float)y;
	w = (float)GetRenderScale(width);
	h = (float)GetRenderScale(height >> 1);
